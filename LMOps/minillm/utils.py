...
def get_model_bck(args, device: int) -> PreTrainedModel:
    config = AutoConfig.from_pretrained(args.model_path)
...
def load_parallel(model: nn.Module, load_dir: str):
    ...
    checkpoint_name = os.path.join(
        load_dir, 
        # f"mp{mpu.get_model_parallel_world_size()}", # commented by Gaeun
        # f"pytorch_model_{mp_rank}.bin" # commented by Gaeun
        )
    assert os.path.exists(checkpoint_name), f"{checkpoint_name} does not exist."
    model = load_checkpoint_and_dispatch(
        model=model, 
        checkpoint=checkpoint_name, 
        # device_map={"": torch.cuda.current_device()}, # commented by Gaeun
        device_map="auto", # added by Gaeun; to automatically distribute GPUs
        dtype=torch.float16)
    dist.barrier()
