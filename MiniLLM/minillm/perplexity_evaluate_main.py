...
from perplexity_metric import compute_perplexity
...
def evaluate_main(args, tokenizer: PreTrainedTokenizer | PreTrainedTokenizerFast, 
    ...
    all_responses = []
    with open(os.path.join(args.save, "answers.json"), "w") as f:    
    ...
    gen_perplexity_res = compute_perplexity(all_responses)

    mean_gen_length = np.mean([len(tokenizer.encode(s)) for s in response_strs])

    log_str = f"{split} | name: {args.data_names} | {gen_res} | {gen_perplexity_res} | lm_loss {round(lm_loss, 4)} | avg. gen lenth: {mean_gen_length}"
    print_rank(log_str)
